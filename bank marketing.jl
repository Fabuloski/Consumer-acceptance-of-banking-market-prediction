...
begin
	...
	barplot!(1:2, [Data_diff[:,1][1],Data_diff[1,2][1]], color = colors[1:2])
	fig
end
...
begin
	...
	local fig = Figure()
	local ax = Axis(fig[1, 1], xticks = (1:2, ["no", "yes"]), ylabel = "Fraction of clients", title = "Class distribution of clients")
	barplot!(1:2, class_dist[:, "# of clients"]/total_clients, color =colors[1:2] )
	fig
end
...
Our hypothesis is that marital status, age, job and education can be used to distinguish between yes and no desired target (If a client agrees to the term deposit or not) with reasonable accuracy. We could be wrong about this, but all would To test this hypothesis, we draw a bar plot such that:
...
begin
	local fig = Figure()
	local ax = Axis(fig[1,1]),# xticklabels=(1:10, keys(month_order)))
...
# ╔═╡ 8b134add-695d-4167-8675-2912d2ca8ca9
y_base = zeros(Int64, nrow(data))

# ╔═╡ 4a75fb9b-5293-4452-80b1-e630e6ea4a80
sum(y.==y_base)/length(y)

# ╔═╡ 0bc921d9-a475-4199-b34d-4847bc543884
begin
...
begin
	local fig = Figure()
	local ax = Axis(fig[1,1], xticks = (1:3, ["Accuracy", "Precision", "Recall"]),
...
!!! note \" \"
	More tuning is required here on our SVM algorithmn due of our class imbalance to get better data for our confusion matrix. we do this by specifying `class_weight` parameter, further reading can be done [here](https://scikit-learn.org/stable/modules/svm.html). 
"
...
begin
	...
	for (i, c) in enumerate(cs)
		...
		algo = SVC(kernel ="rbf", C=c, class_weight="balanced")
		for (ik, (id_train, id_val)) in enumerate(cv_ids)
			algo.fit(X_train[id_train, :], y_train[id_train])
			y_pred1 = algo.predict(X_train[id_val, :])
			precision[ik] = precision_score(y_train[id_val], y_pred1)
			recall[ik] = recall_score(y_train[id_val], y_pred1)
		end
...
begin
	...
	vlines!([1], linestyle=:dash, color=:red)
	axislegend()
	fig
end
...
begin
	clf = SVC(kernel ="rbf", C=10^1, class_weight="balanced")
...
# ╔═╡ 088464c6-25fb-4bff-999a-bfe745bd0bf2
new_per_imp = sum(per_imp["importances"], dims = 2)/2
...
begin
	...
	ax1 = Axis(fig1[1,1], 
			   title = "Ten Most Important Features", 
		   	   ylabel = "The ten most important features", 
		  	   xlabel = "the importance score of features", 
		      yticks=(1:10, top10_most_imp_feat[:, "feat_name"]))
	
	barplot!(ax1, ys, xs, color=ColorSchemes.nipy_spectral[xs], direction=:x, bar_labels = round.(top10_most_imp_feat[:, "perm_importance_score"], digits=4))
	xlims!(0, maximum(xs)+0.01)
	fig1
...
# ╔═╡ c74cae07-1749-4335-9d1c-2afb735c587a
cat(check[:, "Accuracy"], check[:, "Precision"], check[:, "Recall"], dims = (1, 1))
...
begin
	local fig = Figure()
	ax = Axis(fig[1,1], xticks = (1:3, ["Accuracy", "Precision", "Recall"]),
